...
donateNowButton.addEventListener("click", function () {
  ...
  if (!isNaN(inputbox) && inputbox > 0) {
    totalDonation += inputbox;
    const mainAmount = parseFloat(mainBalance.innerText);
    mainBalance.innerText = (mainAmount - totalDonation).toFixed(2);
    const donateBox1 = document.getElementById("donate-box-1");
    donateBox1.innerText = totalDonation.toFixed(2);
  } else {
